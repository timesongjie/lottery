...
/*
 * $(document).ready(function(){ setInterval('AutoScroll(".roll-box")',2000) });
 */
...
$(function() {
	...
	/*
	 * // 页面加载向服务器发送ajax请求,检测是否登录 login=false; // login=true; if (!login) { //
	 * $(".roll-bg,.draw,.count").animate({show()},500); //
	 * $(".roll-bg,.count").hide(); // $(".login").text("您必须登录才能参加抽奖！") //
	 * $(".login").slideToggle(500); return; } ;
	 */
...
//						;
						...
						if (mov) {
							...
							$.ajax({
								...
								success : function(data) {
									...
									if (prize == 2) {
										rotateFunc(2, 126, '恭喜您抽中的二等奖!兑奖SN码为'
												+ sncode);
									}
									if (prize == 3) {
										rotateFunc(3, 246, '恭喜您抽中的三等奖!兑奖SN码为'
												+ sncode);
									}
									if (prize == 0) {
										var angle = [ 36, 66, 96, 152, 180,
												210, 270, 300, 330 ];
	...
	// 弹窗
	/*
	 * cur = 0; $(".small-img-box img").click(function () { // 滚动条回到顶部
	 * $("html,body").animate({scrollTop:0},200); cur = $(this).index(); //
	 * alert(cur); $(".black_overlay,.white_content").show();
